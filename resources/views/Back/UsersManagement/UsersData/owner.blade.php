    ...
    <div class="col-12 col-md-12 col-lg-12">
        <div class="card">
          <div class="card-header">
            <form method="GET" class="form-inline">
              <div class="form-group">
                <input type="text" name="search" class="form-control" placeholder="Cari Owner" value="{{ request()->get('search') }}">
              </div>
              <div class="form-group">
          ...
          <div class="card-header">
            ...
            <button type="button" data-toggle="modal" data-target="#addData" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah Owner</button>
            @endif
          </div>
          <div class="counter">
          ...
          <div class="card-body table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                @forelse($owner as $key => $o)
                <tr>
                  <td align="center">{{ $owner->firstItem() + $key }}</td>
                  <td style="padding-top: 10px; padding-bottom: 10px;" align="center">
                    @if($o->avatar == !NULL)
                      <a href="{{url('assets/images/users/avatar/'.$o->avatar)}}" class="zoom">
                        <img src="{{url('assets/images/users/avatar/'.$o->avatar)}}" class="rounded-circle mr-1" width="100px" height="100px">
                      </a>
                    @else
                  ...
                  <td align="center">{{ $o->gender }}</td>
                  <td align="center">{{ $o->phone_number }}</td>
                  <td align="center">{{ $o->address }}</td>
                  <td align="center">{{ $o->identity }}</td>
                  <td align="center">{{ $o->created_at }}</td>
                  <td align="center">
                    <a href="{{url('/admin-panel/owners/'.$o->id. '/edit')}}">
                      <button type="button" class="btn btn-outline-primary" data-toggle="tooltip" data-placement="left" title="Edit">
                        <i class="fas fa-edit"></i>
                      </button>
                    </a>
                    &nbsp;
                    <a href="{{url('/admin-panel/owners/'.$o->id. '/delete')}}">
            ...
            <div class="pull-right">{{ $owner->links() }}</div>
          </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content"> 
      <div class="modal-header">
        <h5 class="modal-title" id="DataLabel">Tambah Data Owner</h5>
      </div>
      <div class="modal-body">
    <form action="{{url('/admin-panel/owners/add')}}" method="POST">
    {{csrf_field()}}
    <!-- Nama -->
      <div class="form-group">
